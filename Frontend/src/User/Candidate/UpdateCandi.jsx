...
function UpdateCandi({item}){
    const {
      ...
      const onSubmit = async (data) =>{
        ...
        const cleanToken = token.replace(/^"|"$/g, ''); // Remove any surrounding quotes
         ...
         await axios.put(`http://localhost:3000/Candidate/${item._id}`,Userinfo,
          {
          headers: {
 ...
 <dialog id="my_modal_5" className="modal">
  <div className="modal-box">
    ...
    <h3 className="font-bold text-lg">Update Candidate</h3>
   {/* name */}
  ...
  <div className="flex justify-around mt-4">
    <button className="bg-blue-500 text-white rounded-md px-3 py-1 hover:bg-blue-700 duration-200">update</button>
  </div>
  </form>
  </div>
